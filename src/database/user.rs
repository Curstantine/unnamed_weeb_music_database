use sea_query::{Expr, PostgresQueryBuilder, Query, Func, Alias};
use sqlx::PgPool;
use ulid::Ulid;
...
use crate::{
    ...
    models::{
        refresh_token::{RefreshToken, RefreshTokenIden},
        user::{User, UserIden, AccessLevel},
    },
...
pub async fn login(email: Option<String>, username: Option<String>, password: String, db: &PgPool) -> Result<LoginToken, Error> {
...
pub async fn create_user(
	...
	if user.is_some() {
		return Err(Error::new("USER_ALREADY_EXISTS", hyper::StatusCode::BAD_REQUEST));
	}
